...
if [ "$#" -gt "2" ]
then
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table  $destinationdb.customer select * from customer; analyze table $destinationdb.customer compute statistics; analyze table $destinationdb.customer compute statistics for columns;"
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table  $destinationdb.supplier select * from supplier; analyze table $destinationdb.supplier compute statistics; analyze table $destinationdb.supplier compute statistics for columns;"
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table  $destinationdb.part select * from part; analyze table $destinationdb.part compute statistics; analyze table $destinationdb.part compute statistics for columns;"
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table  $destinationdb.date_dim select * from date_dim; analyze table $destinationdb.date_dim compute statistics; analyze table $destinationdb.date_dim compute statistics for columns;"
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table  $destinationdb.time_dim select * from time_dim; analyze table $destinationdb.time_dim compute statistics; analyze table $destinationdb.time_dim compute statistics for columns;"
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table  $destinationdb.lineorder select * from lineorder; analyze table $destinationdb.lineorder compute statistics; analyze table $destinationdb.lineorder compute statistics for columns;"
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "set hive.merge.tezfiles=true; insert overwrite table $destinationdb.flat_lineorder
