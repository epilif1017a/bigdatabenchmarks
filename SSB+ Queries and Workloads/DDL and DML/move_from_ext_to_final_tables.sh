...
then
    ...
    beeline -u "jdbc:hive2://$server:$port/$externaldb;serviceDiscoveryMode=zooKeeper;zooKeeperNamespace=hiveserver2" -e "insert overwrite table $destinationdb.flat_lineorder
                ...
                JOIN  date_dim cd ON l.commitdate = cd.datekey; alter table $destinationdb.flat_lineorder concatenate; analyze table $destinationdb.flat_lineorder compute statistics; analyze table $destinationdb.flat_lineorder compute statistics for columns;"
